...
<template>
    <view class="sy-calendar" catchtouchmove>
        <sy-loading :value="loading" column text="加载中"/>
        ...
        <view class="toolbar sy-ui-border-top">
            <view class="button flex-center sy-ui-icon-arrow-left" @tap="handleViewChange(0)"/>
            <view class="label" @click="handleDateTypeChange">
                <view class="text">{{ viewDate }}</view>
                <view class="icon sy-ui-icon-arrow-right-fill" />
            </view>
            <view class="button flex-center sy-ui-icon-arrow-right" @tap="handleViewChange(1)"/>
        </view>
        <sy-gesture-swiper
            ref="gestureSwiper"
...
<script>
    ...
    export default {
        ...
        props: {
            ...props,
            provideKey: { default: 'calendarConfig' }
        },
        data() {
            return {
                // 字典
        ...
        computed: {
            ...
            markers() {
                if (this.provideData) {
                    return this.provideData.marker || {}
                }
                return {}
            },
            // 视图日期
