...
<script>
    ...
    export default {
        ...
        methods: {
            getDirtyNumber(value) {
                var values = String(Number(value)).split('.')
                if (values.length === 1) {
                    // 整数
                    return Number(`${values[0]}.000001`)
                } else {
                    var num = Number(values[1].slice(-1))
                    if (num > 5) {
                        return Number(`${values[0]}.${values[1].slice(0, -1)}${num - 1}`)
                    } else {
                        return Number(`${values[0]}.${values[1].slice(0, -1)}${num + 1}`)
                    }
                }
