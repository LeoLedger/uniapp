            ...
            >
                <view class="item-wrap" @tap="handleGotoHandle(item)">
                    ...
                    <view class="foot slenderBorderTop">
                        <view class="info">
                            <view class="rows folded f1">反馈异常：{{ item.clientFeedback }}</view>
                        </view>
    ...
    export default {
        ...
        methods: {
            ...
            async queryBaseData() {
                ...
                try {
                    let data = await repairBillsApi.getSendRepairDetail(this.filterConfig.data.id)
                    let productOptions = []
            ...
            // 筛选数据
            filterData() {
                let { imei, productId, secondRepair, billsStatus } = this.filterConfig.data
                let list = this.dataList.filter(item => {
                    return (this.isEmpty(imei) || item.imei.indexOf(imei) > -1) &&
                        ...
                        (this.isEmpty(secondRepair) || item.secondRepair === secondRepair) &&
                ...
                this.tabsData.forEach((tab, index) => {
                    ...
                    case 0:
                        ...
                        break
                    case 1:
                        tab.list = list.filter(v => v._flag === 1)
                        break
                    case 2:
                        tab.list = list.filter(v => v._flag === 0)
                        break
                    }
                    tab.value = tab.list.length
                })
                this.viewList = this.tabsData[this.tabActive].list
                this.isSubmit = this.tabsData[0].value === this.tabsData[1].value
            ...
            async handleSubmit() {
                this.showLoading()
                await repairBillsApi.updateSendRepair({
                    ...this.formData,
                    operType: 2,
                    updateDevices: this.dataList
                })
                this.showToast('保存成功', {
                    mask: true
                })
            ...
            handleGotoHandle(data) {
                let params = {
                    ...
                    isEdit: this.formData.status === 0,
                    billId: this.formData.guidId,
                    excludes: this.dataList.map(v => v.replaceCardId).filter(v => v !== data.replaceCardId).filter(v => v)
                }
